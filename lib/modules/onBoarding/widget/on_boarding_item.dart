...
import '../../Customer/singleCustomerProduct/customer_home_screen.dart';
        ...
        BlueButton(
            ...
            onpress: () {
              ShopCubit.get(context).customerToken =
                  CashHelper.getData("customerToken");
              ShopCubit.get(context).id = CashHelper.getData("customerId");

              if (ShopCubit.get(context).customerToken == null) {
                navigateToFinish(context, Login());
              } else {
                navigateToFinish(
                    context,
                    CustomerHome(
                      id: ShopCubit.get(context).id,
                    ));
                print(ShopCubit.get(context).id);
              }
            })
      ],
    );
